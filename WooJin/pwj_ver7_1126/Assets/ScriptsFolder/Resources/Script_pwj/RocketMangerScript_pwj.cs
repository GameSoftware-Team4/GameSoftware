...
public class RocketMangerScript_pwj : MonoBehaviour
{
	...
	void Start()
	{
		...
		/*for(int i =0;i<3; i++)
        {
			...
			// Debug.Log(s);
			shotPosChild[i] = GameObject.Find("RocketManager").transform.Find("Rocket" + s).transform.gameObject.transform;
			// Debug.Log(GameObjectet" + s).transform.Find("RocketSpawner").gameOb.Find("RocketManager").transform.Find("Rockject.name);
		}*/

		/*foreach (Transform child in shotPosChild)
		{
