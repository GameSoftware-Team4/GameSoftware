...
public class SpawningPool_jhj : MonoBehaviour
{
    ...
    public float keepmonstercnt_jhj; //유지해야할 몬스터 개체수를 저장할 변수
    ...
    {
        if (monstercnt_jhj < keepmonstercnt_jhj)  // 개체수가 유지수 보다 적을 경우
        {
            monstercnt_jhj++;
            var mob = ObjectPool_jhj.GetObject();    // objectPooling
            mob.transform.position = spwanPosArray_jhj[Random.Range(0, 2)].transform.position; //mob의 위치 부여
